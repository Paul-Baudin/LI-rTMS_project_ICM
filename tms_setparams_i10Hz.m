...
elseif isunix
    ...
    rootpath_data       = '/network/lustre/iss01/charpier/analyses/tms/raw/i10Hz/';
elseif ispc
    rootpath_analysis	= '\\lexport\iss01.charpier\analyses\tms\';
    rootpath_data       = '\\lexport\iss01.charpier\analyses\tms\raw\i10Hz\';
else
    error('Platform not supported')
end

datasavedir  = fullfile(rootpath_analysis,'data', 'i10Hz'); %where to save matlab data
imagesavedir = fullfile(rootpath_analysis,'image', 'i10Hz'); %where to save images
...
configcommon.artrm.chan     = {'VmArtRem', 'Im'};
...
config{1}.prefix                            = '1511-'; %leave the '-' at the end
config{1}.readCED.datapath                  = fullfile(rootpath_data,'1511_25082021.smrx'); %with the extension
...
config{2}.prefix                            = '1990-'; %leave the '-' at the end
config{2}.readCED.datapath                  = fullfile(rootpath_data,'1990_18082021.smrx'); %with the extension
...
config{3}.prefix                            = '2012-'; %leave the '-' at the end
config{3}.readCED.datapath                  = fullfile(rootpath_data,'2012_07082021.smrx'); %with the extension
...
config{4}.prefix                            = '2433-'; %leave the '-' at the end
config{4}.readCED.datapath                  = fullfile(rootpath_data,'2433_20082021.smrx'); %with the extension
...
config{5}.prefix                            = '2773-'; %leave the '-' at the end
config{5}.readCED.datapath                  = fullfile(rootpath_data,'2773_25082021.smrx'); %with the extension
...
config{6}.prefix                            = '3257-'; %leave the '-' at the end
config{6}.readCED.datapath                  = fullfile(rootpath_data,'3257_22012021.smrx'); %with the extension
